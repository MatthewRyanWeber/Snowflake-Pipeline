...
- 2026-07-23 — **Fully-in-Snowflake pipeline + native governance.** `sql/40_native/`:
  Dynamic Data Masking policies (`GOV.mask_ssn/phone`) + `PII_READER` role (verified live:
  same SQL-Server row masked for PIPELINE_ROLE, clear for PII_READER), transfer/audit log
  (`GOV.LOAD_LOG`, loader writes each transfer), and Dynamic Tables (`STAGING.dt_*`,
  declarative transform). Tested end-to-end with the local SQL Server (500 raw rows loaded,
  policy-governed). Everything after landing runs natively in Snowflake; the connector is the
  only external piece (Snowflake can't reach a local DB). README/description/docs/sizzler
  updated. 16/16 DQ, 22 tests.
